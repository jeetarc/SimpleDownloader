...
public class EtaHelper {
	...
	public synchronized long update(long speedBytesPerSecond, long bytesDownloaded, long totalBytes) {
		...
		if (progressed) {
			lastBytes = bytes;
			lastProgressNs = now;
			if (lastEta == ETA_STALLED) lastEta = ETA_UNKNOWN;
		}
	...
	private void addSpeedSample(long speed) {
		...
		double effectiveAlpha = Math.max(ALPHA, 1.0 / sampleCount);
		emaSpeed = (effectiveAlpha * speed) + ((1.0 - effectiveAlpha) * emaSpeed);
	}
